...
(function (window) {
  var prms = {
    angularSpeed: 0, //0.01,//Math.PI / 10;,
    frequency: 1, // ajoute un point tous le N rafraîchissements
    penPosition: { x: 0, y: 0 },
    point: {
      color: '#ff0000',
      lineWidth: 1,
  ...
  function draw() {
    ...
    currentAngleStep = currentAngleStep + prms.angularSpeed;
    ...
    o = rotatePoint({ x: initialcX, y: initialcY }, beta);

    drawInnerCircle(ctx, o, currentAngle);

    prms.penPosition = { x: 20, y: -prms.dimensions.innerCircleRadius + 20 };

    if (inking && (step % prms.frequency === 0)) addPoint(currentPointPosition(prms.penPosition, o, currentAngle));
  ...
  function stopCircle() { prms.angularSpeed = 0; }
  ...
  function drawPoint(ctx, point) {
    ...
    ctx.moveTo(point.x + prms.point.xOffset, point.y + prms.point.yOffset);
    ctx.lineTo(point.x + prms.point.xOffset + 1, point.y + prms.point.yOffset);
    ctx.stroke();
  }

  // function drawPointCircle(ctx, point) {
  //   ctx.beginPath();
  //   ctx.lineWidth = point.lineWidth;
  //   ctx.strokeStyle = point.color;
  //   ctx.fillStyle = point.color;
  ...
  canvasElt.addEventListener('click', setPen, false);
  ...
  window.pottingWheelPrms.stopCircle = stopCircle;
